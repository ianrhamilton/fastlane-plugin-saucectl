...
describe Fastlane::Actions::SauceConfigAction do
  describe 'saucelabs config action' do
    ...
    it 'should create config.yml file for android espresso based on user specified virtual device configurations' do
      ...
      expected_config = { "apiVersion" => "v1alpha", "kind" => "espresso", "retries" => 0, "sauce" => { "region" => "eu-central-1", "concurrency" => 1, "metadata" => { "name" => "testing/ios-ui-test-quarantine-15", "build" => "Release " } }, "espresso" => { "app" => "/Users/ian.hamilton/Documents/workspace/fastlane-plugin-rsaucectl/my-demo-app-android/myTestApp.apk", "testApp" => "/Users/ian.hamilton/Documents/workspace/fastlane-plugin-rsaucectl/my-demo-app-android/myTestRunner.apk" }, "artifacts" => { "download" => { "when" => "always", "match" => ["junit.xml"], "directory" => "./artifacts/" } }, "reporters" => { "junit" => { "enabled" => true } }, "suites" => [{ "name" => "testing/ios-ui-test-quarantine-15-logintest", "testOptions" => { "class" => "com.saucelabs.mydemoapp.android.view.activities.LoginTest", "clearPackageData" => true, "useTestOrchestrator" => true }, "emulators" => [{ "name" => "Android GoogleApi Emulator", "orientation" => "portrait", "platformVersions" => ["10.0", "11.0"] }] }, { "name" => "testing/ios-ui-test-quarantine-15-webviewtest", "testOptions" => { "class" => "com.saucelabs.mydemoapp.android.view.activities.WebViewTest", "clearPackageData" => true, "useTestOrchestrator" => true }, "emulators" => [{ "name" => "Android GoogleApi Emulator", "orientation" => "portrait", "platformVersions" => ["10.0", "11.0"] }] }] }
    ...
    it 'ios: should create real device config.yml file' do
      ...
      config_folder = File.expand_path("../", "#{__dir__}")
      config = YAML.load_file("#{config_folder}/.sauce/config.yml")
      expected_config = { "apiVersion" => "v1alpha", "kind" => "xcuitest", "retries" => 0, "sauce" => { "region" => "eu-central-1", "concurrency" => 1, "metadata" => { "name" => "testing/ios-ui-test-quarantine-15", "build" => "Release " } }, "xcuitest" => { "app" => "/Users/ian.hamilton/Documents/workspace/fastlane-plugin-rsaucectl/my-demo-app-ios/MyTestApp.ipa", "testApp" => "/Users/ian.hamilton/Documents/workspace/fastlane-plugin-rsaucectl/my-demo-app-ios/MyTestAppRunner.ipa" }, "artifacts" => { "download" => { "when" => "always", "match" => ["junit.xml"], "directory" => "./artifacts/" } }, "reporters" => { "junit" => { "enabled" => true } }, "suites" => [{ "name" => "testing/ios-ui-test-quarantine-15-my_demo_appuitests", "testOptions" => { "class" => "MyDemoAppUITests.My_Demo_AppUITests" }, "devices" => [{ "name" => "iphone one", "orientation" => "portrait", "options" => { "deviceType" => "PHONE", "private" => false } }] }, { "name" => "testing/ios-ui-test-quarantine-15-my_demo_othertests", "testOptions" => { "class" => "MyDemoAppUITests.My_Demo_OtherTests" }, "devices" => [{ "name" => "iphone one", "orientation" => "portrait", "options" => { "deviceType" => "PHONE", "private" => false } }] }, { "name" => "testing/ios-ui-test-quarantine-15-my_demo_appuitests", "testOptions" => { "class" => "MyDemoAppUITests.My_Demo_AppUITests" }, "devices" => [{ "id" => "iphone_two", "orientation" => "portrait", "options" => { "deviceType" => "PHONE", "private" => false } }] }, { "name" => "testing/ios-ui-test-quarantine-15-my_demo_othertests", "testOptions" => { "class" => "MyDemoAppUITests.My_Demo_OtherTests" }, "devices" => [{ "id" => "iphone_two", "orientation" => "portrait", "options" => { "deviceType" => "PHONE", "private" => false } }] }] }
      expect(config).to eql expected_config
    end

    it 'ios: should create real device config.yml file with test distribution method as shard' do
      Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'ios',
                         kind: 'xcuitest',
                         app_path: '#{File.expand_path("my-demo-app-ios")}',
    ...
    it 'ios: should raise an error when user specifies invalid test kind' do
      expect do
        Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'ios',
                         kind: 'espresso',
    ...
    it 'android: should raise an error when user specifies invalid test kind' do
      expect do
        Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'android',
                         kind: 'xcuitest',
    ...
    it 'android: should raise an error when user does not specify a device array' do
      expect do
        Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'android',
                         kind: 'espresso',
    ...
    it 'ios: should raise an error when user does not specify a testPlan or testTarget' do
      expect do
        Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'ios',
                         kind: 'xcuitest',
    ...
    it 'ios: should create real device config.yml file based on xcode test target' do
      Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'ios',
                         kind: 'xcuitest',
                         app_path: '#{File.expand_path("my-demo-app-ios")}',
    ...
    it 'ios: should create real device config.yml file based on xcode test plan' do
      Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'ios',
                         kind: 'xcuitest',
                         app_path: '#{File.expand_path("my-demo-app-ios")}',
