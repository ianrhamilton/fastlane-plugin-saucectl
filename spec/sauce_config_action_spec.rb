...
describe Fastlane::Actions::SauceConfigAction do
  describe 'saucelabs runner action' do
    ...
    it 'should create config.yml file for android espresso based on user specified virtual device configurations testing by size' do
      Fastlane::FastFile.new.parse("lane :test do
          sauce_config({ platform: 'android',
                         kind: 'espresso',
                         app: '#{File.expand_path("my-demo-app-android")}/myTestApp.apk',
      ...
      config_folder = File.expand_path("../", "#{__dir__}")
      config = YAML.load_file("#{config_folder}/.sauce/config.yml")
      expected_config = {"apiVersion"=>"v1alpha", "kind"=>"espresso", "retries"=>0, "sauce"=>{"region"=>"eu-central-1", "concurrency"=>1, "metadata"=>{"name"=>"unit-test-123", "build"=>"Release "}}, "espresso"=>{"app"=> File.expand_path("my-demo-app-android/myTestApp.apk"), "testApp"=> File.expand_path("my-demo-app-android/myTestRunner.apk")}, "artifacts"=>{"download"=>{"when"=>"always", "match"=>["junit.xml"], "directory"=>"./artifacts/"}}, "reporters"=>{"junit"=>{"enabled"=>true}}, "suites"=>[{"name"=>"unit-test-123-@largetest", "testOptions"=>{"size"=>"@LargeTest", "clearPackageData"=>true, "useTestOrchestrator"=>true}, "emulators"=>[{"name"=>"Android GoogleApi Emulator", "orientation"=>"portrait", "platformVersions"=>%w[10.0 11.0]}]}]}
      expect(config).to eql expected_config
    end
    ...
    it 'should create real device config.yml file for ios real devices using xcode test target and distribution method set as shard' do
      ...
      config_folder = File.expand_path("../", "#{__dir__}")
      config = YAML.load_file("#{config_folder}/.sauce/config.yml")
      expected_config = {"apiVersion"=>"v1alpha", "kind"=>"xcuitest", "retries"=>0, "sauce"=>{"region"=>"eu-central-1", "concurrency"=>1, "metadata"=>{"name"=>"unit-test-123", "build"=>"Release "}}, "xcuitest"=>{"app"=> File.expand_path("my-demo-app-ios/MyTestApp.ipa"), "testApp"=> File.expand_path("my-demo-app-ios/MyTestAppRunner.ipa")}, "artifacts"=>{"download"=>{"when"=>"always", "match"=>["junit.xml"], "directory"=>"./artifacts/"}}, "reporters"=>{"junit"=>{"enabled"=>true}}, "suites"=>[{"name"=>"unit-test-123-shard 1", "testOptions"=>{"class"=>["MyDemoAppUITests.NavigationTest", "MyDemoAppUITests.ProductDetailsTest"]}, "devices"=>[{"name"=>"iPhone RDC One", "orientation"=>"portrait", "options"=>{"carrierConnectivity"=>false, "deviceType"=>"PHONE", "private"=>true}}]}, {"name"=>"unit-test-123-shard 2", "testOptions"=>{"class"=>["MyDemoAppUITests.ProductListingPageTest"]}, "devices"=>[{"id"=>"iphone_rdc_two", "orientation"=>"portrait", "options"=>{"carrierConnectivity"=>false, "deviceType"=>"PHONE", "private"=>true}}]}]}
      expect(config).to eql expected_config
    end
    ...
    it 'should allow users to specify an array of classes to execute on rdc' do
      ...
      expected_config = {"apiVersion"=>"v1alpha", "kind"=>"espresso", "retries"=>0, "sauce"=>{"region"=>"eu-central-1", "concurrency"=>1, "metadata"=>{"name"=>"unit-test-123", "build"=>"Release "}}, "espresso"=>{"app"=> File.expand_path("my-demo-app-android/myTestApp.apk"), "testApp"=> File.expand_path("my-demo-app-android/myTestRunner.apk")}, "artifacts"=>{"download"=>{"when"=>"always", "match"=>["junit.xml"], "directory"=>"./artifacts/"}}, "reporters"=>{"junit"=>{"enabled"=>true}}, "suites"=>[{"name"=>"unit-test-123-iphone rdc one", "testOptions"=>{"class"=>["com.some.package.testing.SomeClassOne", "com.some.package.testing.SomeClassTwo", "com.some.package.testing.SomeClassThree", "com.some.package.testing.SomeClassFour"], "clearPackageData"=>true, "useTestOrchestrator"=>true}, "devices"=>[{"name"=>"iPhone RDC One", "orientation"=>"portrait", "options"=>{"carrierConnectivity"=>false, "deviceType"=>"PHONE", "private"=>true}}]}]}
    ...
    it 'should allow ios users to specify an array of classes to execute on rdc' do
      ...
      expected_config = {"apiVersion"=>"v1alpha", "kind"=>"xcuitest", "retries"=>0, "sauce"=>{"region"=>"eu-central-1", "concurrency"=>1, "metadata"=>{"name"=>"unit-test-123", "build"=>"Release "}}, "xcuitest"=>{"app"=> File.expand_path("my-demo-app-ios/myTestApp.app"), "testApp"=> File.expand_path("my-demo-app-ios/myTestRunner.app")}, "artifacts"=>{"download"=>{"when"=>"always", "match"=>["junit.xml"], "directory"=>"./artifacts/"}}, "reporters"=>{"junit"=>{"enabled"=>true}}, "suites"=>[{"name"=>"unit-test-123-iphone rdc one", "testOptions"=>{"class"=>["MyDemoAppUITests.SomeClassOne", "MyDemoAppUITests.SomeClassTwo", "MyDemoAppUITests.SomeClassThree", "MyDemoAppUITests.SomeClassFour"]}, "devices"=>[{"name"=>"iPhone RDC One", "orientation"=>"portrait", "options"=>{"carrierConnectivity"=>false, "deviceType"=>"PHONE", "private"=>true}}]}]}
      expect(config).to eql expected_config
    end
