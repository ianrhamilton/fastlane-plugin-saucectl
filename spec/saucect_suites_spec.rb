  ...
  describe 'suites' do
    ...
    it 'should raise an error when user does not specify device array' do
      ...
      expect { Fastlane::Saucectl::Suites.new(@config).is_ios_reqs_satisfied? }
        .to raise_error(StandardError, "❌ For ios you must specify test_target or testPlan")
    end
    ...
    it 'should create an array of suites for virtual devices based on specified test distribution type' do
    ...
    it 'should not include android values when testing on ios platform' do
      @config['platform'] = 'ios'
    ...
    it 'should distribute an array of test cases to real device suites based on number of specified shards' do
      @config['platform'] = 'android'
      @config['shards'] = 3
      @config['test_distribution'] = 'shard'
      ...
      expect(actual_test_options.size).to eql(3)
    end

    it 'should raise an error when user specifies invalid device type' do
      @config['platform'] = 'android'
